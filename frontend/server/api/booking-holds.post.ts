import type { BookingHold, BookingHoldPayload } from "~~/app/types/booking"
...
export default defineEventHandler(async (event) => {
  try {
    return await authenticatedDjangoRequest<BookingHold>(event, "/booking-holds/", {
      method: "POST",
      body: await readBody<BookingHoldPayload>(event),
